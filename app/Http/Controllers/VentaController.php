    ...
    public function store(StoreVentaRequest $request, KardexService $kardexService)
{
    try {
        ...
        DB::transaction(function () use ($request, &$aperturaVenta, $kardexService, $datosVenta, &$ventaCreada) {
        ...
        if (! $ventaCreada) {
            return response()->json([
                'status' => 'error',
                'message' => 'La venta no pudo completarse correctamente. Intente de nuevo.',
            ], 500);
        }
        ...
        return response()->json([
            'status' => 'error',
            'message' => 'Ocurrió un error interno al procesar la venta. Por favor intente nuevamente o contacte a soporte.' . $e->getMessage(),
        ], 500);
    }
}
